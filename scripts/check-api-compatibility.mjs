...
const BASELINE_APIS = {
    ...
    'SessionManager': {
        ...
        notes: 'Breaking changes from previous version: init->initialize, createNewSession->createSession, etc.'
    },
    'PatternWorkerPool': {
...
function checkApiCompatibility() {
    ...
    for (const [className, config] of Object.entries(BASELINE_APIS)) {
        ...
        for (const method of baselineMethods) {
            if (!currentExports.methods.includes(method)) {
                if (isDocumentedBreakingChange(className, method)) {
                    warnings.push(`${className}: Method '${method}' was removed (documented breaking change)`);
                } else {
        ...
        console.log(`  ✅ Found ${currentExports.methods.length} methods, ${currentExports.constants.length} constants\n`);
    }
