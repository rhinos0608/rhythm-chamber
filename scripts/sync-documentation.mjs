...
function scanCodebase() {
    ...
    const allSourceFiles = findJSFiles(JS_DIR).filter(f =>
        !f.includes('/vendor/') &&
...
function formatStats(stats) {
    ...
    output += `│ ${'Total'.padEnd(24)} ${Object.values(stats.internalModules).reduce((s, x) => s + x.totalFiles, 0).toString().padStart(7)}  ${stats.summary.totalInternalLines.toString().padStart(8)}                   │\n`;
    output += '└──────────────────────────────────────────────────────────────┘\n\n';
