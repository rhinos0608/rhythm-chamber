...
import {
  ...
  DegradationTier,
} from '../../js/services/storage-degradation-manager.js';
...
describe('SESSION_EVENT_SCHEMAS - API Compatibility', () => {
  ...
  it('should have session:created schema', () => {
    ...
    expect(SESSION_EVENT_SCHEMAS['session:created'].payload).toEqual({
      sessionId: 'string',
      title: 'string',
    });
  });
  ...
  it('should have session:loaded schema', () => {
    ...
    expect(SESSION_EVENT_SCHEMAS['session:loaded'].payload).toEqual({
      sessionId: 'string',
      messageCount: 'number',
    });
  });
  ...
  it('should have session:switched schema', () => {
    ...
    expect(SESSION_EVENT_SCHEMAS['session:switched'].description).toBe(
      'Switched to different session'
    );
    expect(SESSION_EVENT_SCHEMAS['session:switched'].payload).toEqual({
  ...
  it('should have session:updated schema', () => {
    ...
    expect(SESSION_EVENT_SCHEMAS['session:updated'].payload).toEqual({
      sessionId: 'string',
      field: 'string',
    });
  });
...
describe('API Breaking Changes - Documentation', () => {
  it('should document SessionManager breaking changes', () => {
    const breakingChanges = {
      ...
      'onSessionUpdate()':
        'Removed - Use EventBus.on with specific events (session:created, session:loaded, etc.) instead',
      ...
      'clearConversationAlias()': 'clearConversation() - Alias for clearAllSessions',
    };
