...
describe('OpenAICompatibleProvider call function', () => {
    it('throws error when config.apiUrl is missing', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.call('test-key', {}, messages)
    ...
    it('throws error when messages array is empty', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.call('test-key', config, [])
    ...
    it('throws error when messages is not an array', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.call('test-key', config, null)
        ).rejects.toThrow('Messages array is required');
    });
    ...
    it('throws error when model is not configured', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.call('test-key', config, messages)
    ...
    it('includes tools in request when provided', async () => {
        ...
        const messages = [{ role: 'user', content: 'Hello' }];
        const tools = [
            { type: 'function', function: { name: 'test_func', parameters: {} } }
        ];
...
describe('OpenAICompatibleProvider callStreaming function', () => {
    ...
    it('processes SSE chunks correctly', async () => {
        ...
        const result = await OpenAICompatibleProvider.callStreaming('test-key', config, messages, [], onToken);
...
describe('OpenAICompatibleProvider Error handling', () => {
    ...
    it('handles HTML error responses', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.call('test-key', config, messages)
        ).rejects.toThrow('API error: 404: <html><body>Not Fo');
    });
...
describe('OpenAICompatibleProvider listModels', () => {
    it('throws error when apiUrl is not configured', async () => {
        await expect(
            OpenAICompatibleProvider.listModels('test-key', {})
    ...
    it('throws error on non-OK response', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.listModels('invalid-key', config)
        ).rejects.toThrow('Failed to list models: 401');
    });
    ...
    it('throws timeout error after configured timeout', async () => {
        ...
        await expect(
            OpenAICompatibleProvider.listModels('test-key', config)
        ).rejects.toThrow();
    });
});
