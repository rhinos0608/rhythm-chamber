...
describe('RecoveryLockManager', () => {
  ...
  describe('Lock Acquisition', () => {
    ...
    it('should throw error when lock acquisition fails', async () => {
      ...
      await expect(lockManager.acquireRecoveryLock('recovery_security')).rejects.toThrow(
        'Cannot acquire recovery lock: recovery_security'
      );
    });
    ...
    it('should warn when lock acquisition fails', async () => {
      ...
      await expect(lockManager.acquireRecoveryLock('recovery_storage')).rejects.toThrow();
  ...
  describe('State Validation', () => {
    ...
    it('should reject recovery in disallowed state', async () => {
      ...
      await expect(lockManager.validateRecoveryState(request)).rejects.toThrow(
        'Recovery not allowed in state: recording'
      );
    });
  ...
  describe('Tab Coordination', () => {
    it('should emit recovery started event', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('RECOVERY:STARTED', {
  ...
  describe('Broadcast Recovery Request', () => {
    ...
    it('should stop delegating when max attempts reached', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('RECOVERY:DELEGATION_EXHAUSTED', {
        recoveryId: 'recovery-123',
        attempts: 3,
      });
    });
    ...
    it('should emit recovery delegated event', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('RECOVERY:DELEGATED', {
        recoveryId: 'recovery-123',
        sourceTabId: 'tab-1',
        delegationAttempt: 1,
      });
  ...
  describe('Tab Leadership Monitoring', () => {
    ...
    it('should subscribe to leadership changes', async () => {
      ...
      expect(mockTabCoordinator.on).toHaveBeenCalledWith('leadership-change', expect.any(Function));
    });
  ...
  describe('Lock Release', () => {
    ...
    it('should handle release failure gracefully', async () => {
      ...
      await expect(
        lockManager.releaseRecoveryLock('recovery_storage', 'lock-id-123')
