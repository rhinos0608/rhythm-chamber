...
describe('CRITICAL: ReDoS Bypass Vulnerability Fix', () => {
  describe('Original bypass pattern detection', () => {
    ...
    it('should detect ((a*)+ - variation with star', () => {
      ...
      expect(
        result.errors.some(
          e => e.includes('nested quantifiers') || e.includes('dangerous construct')
        )
      ).toBe(true);
    });
  ...
  describe('Real-world attack patterns', () => {
    ...
    it(String.raw`should block (\w+\s+)+ - common vulnerable pattern`, () => {
    ...
    it(String.raw`should block ([\w\s]+)+ - another common vulnerable pattern`, () => {
      const result = validateSchema('test', {
        type: 'string',
        pattern: '([\\w\\s]+)+',
      });
  ...
  describe('Multiple quantifiers without nesting', () => {
    ...
    it(String.raw`should allow \\d+\\w* - different tokens with quantifiers`, () => {
