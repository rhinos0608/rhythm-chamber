...
import {
  ...
  ProviderPriority,
  ProviderHealth,
} from '../../js/services/provider-fallback-chain.js';
  ...
  const mockAuthority = {
    ...
    recordSuccess: vi.fn((provider, latencyMs) => {
      ...
      if (status) {
        ...
        status.avgLatencyMs =
          status.avgLatencyMs === 0 ? latencyMs : status.avgLatencyMs * 0.9 + latencyMs * 0.1;
...
describe('ProviderFallbackChain - Characterization Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const { FallbackResponseService } =
      await import('../../js/services/fallback-response-service.js');
  ...
  describe('Circuit Breaker Integration', () => {
    ...
    it('should record success to health authority', async () => {
      ...
      expect(mockAuthority.recordSuccess).toHaveBeenCalledWith(
        expect.any(String),
        expect.any(Number)
      );
    });
