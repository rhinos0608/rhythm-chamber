...
describe('Critical Paths Error Boundary - TDD', () => {
  ...
  describe('Chat Message Sending Errors', () => {
    it('should catch network errors during message sending', async () => {
      const sendMessage = vi.fn().mockRejectedValue(new Error('Network request failed'));
      ...
      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(sendMessage, {
          context: 'sendMessage',
    ...
    it('should catch timeout errors during LLM API calls', async () => {
      const llmCall = vi.fn().mockRejectedValue(new Error('Request timeout after 30000ms'));

      // Expected: Should provide user-friendly timeout message
      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(llmCall, {
          context: 'llmCall',
    ...
    it('should catch validation errors in message processing', async () => {
      const processMessage = vi
        .fn()
        .mockRejectedValue(new Error('Message validation failed: content too long'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(processMessage, {
    ...
    it('should catch and report tool execution errors', async () => {
      const executeTool = vi
        .fn()
        .mockRejectedValue(new Error('DataQuery tool failed: no streams available'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(executeTool, {
  ...
  describe('File Upload Processing Errors', () => {
    it('should catch file size validation errors', async () => {
      const validateFile = vi
        .fn()
        .mockRejectedValue(new Error('File too large: 600MB exceeds 500MB limit'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(validateFile, {
          context: 'fileUpload',
    ...
    it('should catch file parsing errors in worker', async () => {
      const parseFile = vi.fn().mockRejectedValue(new Error('Invalid ZIP file format'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(parseFile, {
          context: 'fileParsing',
    ...
    it('should catch worker initialization errors', async () => {
      const initWorker = vi.fn().mockRejectedValue(new Error('Failed to initialize parser worker'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(initWorker, {
          context: 'workerInit',
    ...
    it('should catch memory warnings during processing', async () => {
      const processChunk = vi.fn().mockRejectedValue(new Error('Memory limit exceeded: 95% usage'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(processChunk, {
  ...
  describe('Session Management Errors', () => {
    it('should catch session save errors', async () => {
      const saveSession = vi.fn().mockRejectedValue(new Error('IndexedDB: Transaction failed'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(saveSession, {
          context: 'sessionSave',
    ...
    it('should catch session load errors', async () => {
      const loadSession = vi.fn().mockRejectedValue(new Error('Session not found: invalid-id'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(loadSession, {
          context: 'sessionLoad',
    ...
    it('should catch session switch errors', async () => {
      const switchSession = vi
        .fn()
        .mockRejectedValue(new Error('Failed to switch session: concurrent modification'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(switchSession, {
    ...
    it('should catch emergency backup errors', async () => {
      const backup = vi.fn().mockRejectedValue(new Error('localStorage: Quota exceeded'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(backup, {
  ...
  describe('LLM API Call Errors', () => {
    it('should catch API authentication errors', async () => {
      const callLLM = vi.fn().mockRejectedValue(new Error('401 Unauthorized: Invalid API key'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(callLLM, {
          context: 'llmApiCall',
          fallback: {
    ...
    it('should catch provider timeout errors', async () => {
      const callLLM = vi
        .fn()
        .mockRejectedValue(new Error('Provider timeout: No response within 60000ms'));
    ...
    it('should catch malformed response errors', async () => {
      const callLLM = vi
        .fn()
        .mockRejectedValue(new Error('Invalid response: missing choices array'));

      const result = ErrorBoundary.wrap
        ? await ErrorBoundary.wrap(callLLM, {
          context: 'llmApiCall',
          fallback: {
            content: 'Received an invalid response from the AI provider.',
            status: 'error',
          },
  ...
  describe('Error Context and Logging', () => {
    ...
    it('should aggregate multiple errors from the same operation', async () => {
      const failingOp = vi
        .fn()
        .mockRejectedValue(new Error('Multiple errors: validation, parsing, API'));
