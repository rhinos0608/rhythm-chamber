...
describe('Vector Dimension Validation (efcc205)', () => {
  it('should validate consistent dimensions across all vectors', () => {
    ...
    // Non-numeric values
    const nonNumericVectors = [{ id: 'vec1', vector: [1, 2, 'three'] }];
      ...
      function checkVectorsWithWarning(vectors) {
        ...
        for (let i = 0; i < vectors.length; i++) {
          ...
          if (!item.vector || !Array.isArray(item.vector)) {
            console.warn(
              `[LocalVectorStore] Invalid vector at index ${i}: missing or not an array`
            );
            return false;
          }
          ...
          if (expectedDimensions === null) {
            ...
            console.warn(
              `[LocalVectorStore] DIMENSION MISMATCH DETECTED at index ${i}: expected ${expectedDimensions}, got ${currentDimensions}. Vector ID: ${item.id}`
            );
            console.warn(
              '[LocalVectorStore] Falling back to slower search path. Consider cleaning up mismatched vectors.'
            );
            return false;
          }
        }
...
describe('Timestamp Validation (efcc205)', () => {
  it('should validate timestamps before creating Date objects', () => {
    function validateTimestamp(timestamp, streamIndex) {
      ...
      if (!timestamp) {
        console.warn(
          `[EmbeddingWorker] Stream missing timestamp, skipping stream at index: ${streamIndex}`
        );
      ...
      if (isNaN(date.getTime())) {
        console.warn(
          `[EmbeddingWorker] Invalid date for stream at index: ${streamIndex}, timestamp: ${timestamp}`
        );
    ...
    try {
      function processStreamsWithValidation(streams) {
        ...
        streams.forEach((stream, idx) => {
          ...
          if (!timestamp) {
            console.warn(
              `[EmbeddingWorker] Stream missing timestamp, skipping stream at index: ${idx}`
            );
          ...
          if (isNaN(date.getTime())) {
            console.warn(
              `[EmbeddingWorker] Invalid date for stream at index: ${idx}, timestamp: ${timestamp}`
            );
            skippedStreams.push(idx);
            return; // Skip this stream
          }
