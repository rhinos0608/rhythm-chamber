...
test.describe('Multi-Tab Coordination', () => {
  test('should show warning when second tab opens', async ({ context }) => {
    ...
    try {
      ...
      const uploadDisabled = await page2
        .locator('#upload-zone')
...
test.describe('File Validation', () => {
  test('should reject invalid JSON with appropriate error', async ({ page }) => {
    ...
    await expect(page.locator('#progress-text')).toContainText(/error|invalid|fail/i, {
      timeout: 10000,
    });
  ...
  test('should reject empty JSON array', async ({ page }) => {
    ...
    // Should show error
    await expect(page.locator('#progress-text')).toContainText(/error|empty|no.*data|fail/i, {
      timeout: 10000,
    });
...
test.describe('Settings Persistence', () => {
  ...
  test('should show settings modal and close properly', async ({ page }) => {
    ...
    const closeBtn = page
      .locator('#settings-modal .close-btn, #settings-modal [class*="close"]')
      .first();
    await closeBtn.click();
    await page.waitForTimeout(300); // Wait for modal to close
    await expect(page.locator('#settings-modal')).not.toBeVisible({ timeout: 5000 });
...
test.describe('Start Over Functionality', () => {
  test('should show reset confirmation modal', async ({ page }) => {
    ...
    const startOverBtn = page
      .locator('#reset-btn, .reset-btn, button:has-text("Start Over")')
      .first();
    ...
    if (await startOverBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const resetModal = page
        .locator('#reset-confirm-modal, #reset-modal, .reset-modal')
        .first();
...
test.describe('Chat Input Functionality', () => {
  test('should have accessible chat input after personality reveal', async ({ page }) => {
    ...
    await expect(
      page.locator('#chat-container, .chat-container, [class*="chat"]').first()
    ).toBeVisible({ timeout: 10000 });

    // Chat input should be visible and enabled
    const chatInput = page
      .locator('#chat-input, textarea[placeholder*="message"], .chat-input')
      .first();
...
test.describe('Sidebar Functionality', () => {
  ...
  test('should persist sidebar state across reload', async ({ page }) => {
    ...
    if (await toggleBtn.isVisible()) {
      ...
      const afterReloadCollapsed = await sidebar.evaluate(el =>
...
test.describe('Error Handling', () => {
  test('should handle malformed JSON gracefully', async ({ page }) => {
    ...
    await expect(page.locator('#progress-text')).toContainText(/error|parse|invalid|json/i, {
      timeout: 10000,
    });
