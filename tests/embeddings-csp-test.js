...
export function runEmbeddingsCSPTest() {
  ...
  function testCSPMetaTag() {
    ...
    if (!cspContent.includes('worker-src')) {
      results.failed.push({
        test: 'CSP worker-src',
        reason:
          'worker-src directive is required for Transformers.js and app workers but not found',
      });
      return false;
    }
  ...
  async function testTransformersLoaded() {
    if (typeof window.transformers === 'undefined') {
      results.failed.push({
        ...
        reason:
          'window.transformers is not defined. Check that js/vendor/transformers.min.js is loaded.',
      });
      return false;
    }
    results.passed.push({
      test: 'Transformers.js Loaded',
      reason: 'window.transformers is available',
    });
    ...
    results.passed.push({
      test: 'Transformers.js pipeline',
      reason: 'pipeline function is available',
    });
  ...
  function testVendorScriptSource() {
    ...
    results.passed.push({
      test: 'Transformers Script Tag',
      reason: 'Loaded from local vendor directory',
    });
  ...
  function testWorkerSupport() {
    try {
      ...
      const testWorker = new Worker(
        URL.createObjectURL(
          new Blob(['done();postMessage("worker-works");'], { type: 'text/javascript' })
        )
      );
  ...
  function testNoCDNNetworkRequests() {
    const jsdelivrRequests = networkRequests.filter(
      req => req.name && req.name.includes('jsdelivr.net')
    );
    ...
    if (jsdelivrRequests.length > 0) {
      ...
      console.error(
        '✗ Found jsDelivr network requests:',
        jsdelivrRequests.map(r => r.name)
      );
      return false;
    }
    ...
    results.passed.push({
      test: 'No CDN Network Requests',
      reason: 'No jsDelivr requests detected',
    });
  ...
  function testCSPEnforced() {
    ...
    results.passed.push({
      test: 'CSP Enforced',
      reason: 'CSP meta tag present and structured correctly',
    });
