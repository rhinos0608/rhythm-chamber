...
function ensureSessionSalt() {
    try {
        ...
        if (!salt) {
            // SECURITY FIX (HIGH Issue #8): Require crypto.getRandomValues
            // Previous implementation fell back to Math.random() which is NOT
            // cryptographically secure and predictable.
            //
            // Math.random() is predictable because:
            // - Uses timestamp as seed in some browsers
            // - Only ~32 bits of entropy
            // - Same value can be generated by an attacker
            //
            // If crypto.getRandomValues is unavailable, we FAIL CLOSED rather than
            // generate a weak salt that would compromise token binding security.
            if (!crypto?.getRandomValues) {
                const errorMsg = 'Secure salt generation requires crypto.getRandomValues. Please use HTTPS or localhost.';
                console.error('[Security]', errorMsg);

                // Dispatch event for UI to show error
                if (typeof window !== 'undefined') {
                    window.dispatchEvent(new CustomEvent('security:crypto-unavailable', {
