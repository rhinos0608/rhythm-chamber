...
const Storage = {
    ...
    async init() {
        return queuedOperation(async () => {
            ...
            await IndexedDBCore.initDatabaseWithRetry({
                onVersionChange: () => {
                    if (isCriticalOperationInProgress()) {
                        console.warn(
                            '[Storage] Version change deferred - critical operation in progress'
                        );
                        setQueuePendingReload(true);
                    } else {
            ...
            QuotaManager.on('threshold_exceeded', async usage => {
                if (usage.percent > 90) {
                    console.log(
                        `[Storage] Quota threshold exceeded (${usage.percent.toFixed(1)}%), triggering auto-archive`
                    );
                    try {
                        const result = await ArchiveService.archiveOldStreams();
                        console.log(
                            `[Storage] Auto-archive complete: ${result.archived} streams archived, saved ${(result.savedBytes / 1024 / 1024).toFixed(2)}MB`
                        );
                    } catch (error) {
    ...
    async clearAllData() {
        ...
        try {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && (key.startsWith('rhythm_chamber_') || key.startsWith('spotify_'))) {
            ...
            const RAG = ModuleRegistry?.getModule ? await ModuleRegistry.getModule('RAG') : null;
    ...
    async getDataSummary() {
        ...
        return {
            ...
            estimatedSizeMB: streams
                ? Math.round((JSON.stringify(streams).length / 1024 / 1024) * 100) / 100
                : 0,
        };
    },
