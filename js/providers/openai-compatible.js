...
async function call(apiKey, config, messages, tools, onProgress = null) {
    ...
    if (!config?.apiUrl) {
        throw new Error('OpenAI-Compatible provider requires apiUrl to be configured. Add "apiUrl" to your provider settings (e.g., "https://api.example.com/v1/chat/completions").');
    }
...
function normalizeToolArguments(args) {
    ...
    if (typeof args === 'string') {
        try {
            ...
            console.error('[OpenAICompatible] Invalid tool arguments JSON string:', args.substring(0, 100));
...
async function handleStreamingResponse(response, onProgress) {
    ...
    while (true) {
        ...
        for (const line of lines) {
            ...
            if (!parsed) {
                console.debug('[OpenAICompatible] Failed to parse chunk, skipping:', data.substring(0, 100));
                continue;
            }
    ...
    console.log('[OpenAICompatible] Streaming complete - content length:', fullContent.length, 'tool_calls:', toolCallsAccumulator.length);
...
async function validateApiKey(apiKey, config) {
    ...
    try {
        ...
        if (response.status === 404) {
            console.warn('[OpenAICompatible] /models endpoint not found, but chat endpoint may still work');
            return true;
        }
